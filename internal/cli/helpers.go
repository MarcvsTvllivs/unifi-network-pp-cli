...
func classifyAPIError(err error, flags *rootFlags) error {
	...
	case strings.Contains(msg, "HTTP 400") && cliutil.LooksLikeAuthError(msg):
		return authErr(fmt.Errorf("%w\nhint: the controller rejected the request — usually a missing/invalid login."+
			...
			"\n      Run 'unifi-network-pp-cli doctor' to check auth status."+
	...
	case strings.Contains(msg, "HTTP 401"):
		return authErr(fmt.Errorf("%w\nhint: controller login failed — check UNIFI_USERNAME/UNIFI_PASSWORD (a LOCAL controller admin, not a UI.com SSO login) and UNIFI_HOST."+
			...
			"\n      Run 'unifi-network-pp-cli doctor' to check auth status.", err))
	case strings.Contains(msg, "HTTP 403"):
		return authErr(fmt.Errorf("%w\nhint: forbidden. Site-scoped controller endpoints need the username/password cookie session; the Integration API key does NOT authenticate /api/s/.../stat|rest|cmd."+
			"\n      Verify UNIFI_HOST/UNIFI_USERNAME/UNIFI_PASSWORD and that the admin account has the needed permissions."+
			"\n      Run 'unifi-network-pp-cli doctor' to check auth status.", err))
