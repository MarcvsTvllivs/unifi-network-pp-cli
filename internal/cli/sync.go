...
Resource scoping:
  ...
  unifi-network-pp-cli sync

  # Sync specific resources only
  unifi-network-pp-cli sync --resources channels,messages

  # Full resync (ignore previous checkpoint)
  unifi-network-pp-cli sync --full

  # Incremental sync: only records from the last 7 days
  unifi-network-pp-cli sync --since 7d

  # Parallel sync with 8 workers
  unifi-network-pp-cli sync --concurrency 8

  # Latest-only: refresh head of each resource, no historical backfill
  unifi-network-pp-cli sync --latest-only`,
	...
	cmd.Flags().StringVar(&dbPath, "db", "", "Database path (default: ~/.local/share/unifi-network-pp-cli/data.db)")
...
func upsertSingleObject(db *store.Store, resource string, data json.RawMessage) error {
	...
	switch resource {
	case "clients":
		return db.UpsertClients(data)
	case "devices":
		return db.UpsertDevices(data)
	case "events":
		return db.UpsertEvents(data)
	case "firewall-groups":
		return db.UpsertFirewallGroups(data)
	case "firewall-policies":
		return db.UpsertFirewallPolicies(data)
	case "networks":
		return db.UpsertNetworks(data)
	case "port-forwards":
		return db.UpsertPortForwards(data)
	case "port-profiles":
		return db.UpsertPortProfiles(data)
	case "routes":
		return db.UpsertRoutes(data)
	case "usergroups":
		return db.UpsertUsergroups(data)
	case "vouchers":
		return db.UpsertVouchers(data)
	case "wlans":
		return db.UpsertWlans(data)
	default:
...
func syncResourcePath(resource string) (string, error) {
	paths := map[string]string{
		"acl-rules":         "/v2/api/site/__SITE__/acl-rules",
		"ap-groups":         "/v2/api/site/__SITE__/apgroups",
		"client-groups":     "/v2/api/site/__SITE__/network-members-groups",
		"clients":           "/api/s/__SITE__/stat/sta",
		"clients-user":      "/api/s/__SITE__/rest/user",
		"content-filters":   "/v2/api/site/__SITE__/content-filtering",
		"devices":           "/api/s/__SITE__/stat/device",
		"dns-records":       "/v2/api/site/__SITE__/static-dns",
		"events":            "/api/s/__SITE__/stat/alarm",
		"firewall-groups":   "/api/s/__SITE__/rest/firewallgroup",
		"firewall-policies": "/v2/api/site/__SITE__/firewall-policies",
		"firewall-zones":    "/v2/api/site/__SITE__/firewall/zone-matrix",
		"networks":          "/api/s/__SITE__/rest/networkconf",
		"oon-policies":      "/v2/api/site/__SITE__/object-oriented-network-configs",
		"port-forwards":     "/api/s/__SITE__/rest/portforward",
		"port-profiles":     "/api/s/__SITE__/rest/portconf",
		"qos-rules":         "/v2/api/site/__SITE__/qos-rules",
		"routes":            "/api/s/__SITE__/rest/routing",
		"traffic-routes":    "/v2/api/site/__SITE__/trafficroutes",
		"usergroups":        "/api/s/__SITE__/rest/usergroup",
		"vouchers":          "/api/s/__SITE__/stat/voucher",
		"wlans":             "/api/s/__SITE__/rest/wlanconf",
	}
