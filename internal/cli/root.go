...
func newRootCmd(flags *rootFlags) *cobra.Command {
	rootCmd := &cobra.Command{
		Use:   "unifi-network-pp-cli",
		Short: `Operate and audit a UniFi Network controller from the shell or an agent — full device/client/WiFi/firewall/routing control plus a local SQLite mirror with cross-resource queries no UniFi tool has.`,
		Long: `Operate and audit a UniFi Network controller from the shell or an agent — full device/client/WiFi/firewall/routing control plus a local SQLite mirror with cross-resource queries no UniFi tool has.
...
Health check: run 'unifi-network-pp-cli doctor' to verify auth and connectivity.
See README.md or the bundled SKILL.md for recipes.`,
		SilenceUsage: true,
		Version:      version,
	}
	rootCmd.SetVersionTemplate("unifi-network-pp-cli {{ .Version }}\n")
	...
	rootCmd.PersistentFlags().StringVar(&flags.profileName, "profile", "", "Apply values from a saved profile (see 'unifi-network-pp-cli profile list')")
	...
	rootCmd.AddCommand(newAclRulesCmd(flags))
	rootCmd.AddCommand(newApGroupsCmd(flags))
	rootCmd.AddCommand(newClientGroupsCmd(flags))
	rootCmd.AddCommand(newClientsCmd(flags))
	rootCmd.AddCommand(newContentFiltersCmd(flags))
	rootCmd.AddCommand(newDevicesCmd(flags))
	rootCmd.AddCommand(newDnsRecordsCmd(flags))
	rootCmd.AddCommand(newDpiCmd(flags))
	rootCmd.AddCommand(newEventsCmd(flags))
	rootCmd.AddCommand(newFirewallGroupsCmd(flags))
	rootCmd.AddCommand(newFirewallPoliciesCmd(flags))
	rootCmd.AddCommand(newNetworksCmd(flags))
	rootCmd.AddCommand(newOonPoliciesCmd(flags))
	rootCmd.AddCommand(newPortForwardsCmd(flags))
	rootCmd.AddCommand(newPortProfilesCmd(flags))
	rootCmd.AddCommand(newQosRulesCmd(flags))
	rootCmd.AddCommand(newRoutesCmd(flags))
	rootCmd.AddCommand(newStatsCmd(flags))
	rootCmd.AddCommand(newSystemCmd(flags))
	rootCmd.AddCommand(newTrafficRoutesCmd(flags))
	rootCmd.AddCommand(newUsergroupsCmd(flags))
	rootCmd.AddCommand(newWlansCmd(flags))
	...
	rootCmd.AddCommand(newNovelClientHuntCmd(flags))
	rootCmd.AddCommand(newNovelFirewallCoverageCmd(flags))
	rootCmd.AddCommand(newNovelPoeBudgetCmd(flags))
	rootCmd.AddCommand(newNovelSinceCmd(flags))
	rootCmd.AddCommand(newNovelTopologyCmd(flags))
	rootCmd.AddCommand(newAPICmd(flags))
	rootCmd.AddCommand(newFirewallZonesPromotedCmd(flags))
	rootCmd.AddCommand(newVouchersPromotedCmd(flags))
	rootCmd.AddCommand(newVersionCliCmd())

	// Hand-authored: attach cmd/{mgr} action commands and toggles to their
	// generated resource parents, plus the switch/vpn groups.
	registerUnifiActions(rootCmd, flags)
...
func (f *rootFlags) newClient() (*client.Client, error) {
	...
	c.DryRun = f.dryRun
	c.NoCache = f.noCache
	// Hand-authored hook: upgrade the generated api_key client to speak the
	// UniFi controller protocol (cookie login + CSRF, /proxy/network prefix,
	// __SITE__ substitution, self-signed TLS). No-op under verify/mock mode.
	client.InstallUnifiTransport(c, cfg)
	return c, nil
}
