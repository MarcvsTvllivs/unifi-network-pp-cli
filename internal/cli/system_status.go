...
func newSystemStatusCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		Use:         "status",
		Short:       "Controller status (server version, up flag)",
		Example:     "  unifi-network-pp-cli system status",
		Annotations: map[string]string{"pp:endpoint": "system.status", "pp:method": "GET", "pp:path": "/api/s/__SITE__/stat/status", "mcp:read-only": "true"},
