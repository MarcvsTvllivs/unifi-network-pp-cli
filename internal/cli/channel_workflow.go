...
and full resync. After archiving, use 'search' for instant full-text search.`,
  ...
  unifi-network-pp-cli workflow archive

  # Full re-archive (ignore previous sync state)
  unifi-network-pp-cli workflow archive --full`,
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			resources := []string{"acl-rules", "ap-groups", "client-groups", "clients", "clients-user", "content-filters", "devices", "dns-records", "events", "firewall-groups", "firewall-policies", "firewall-zones", "networks", "oon-policies", "port-forwards", "port-profiles", "qos-rules", "routes", "traffic-routes", "usergroups", "vouchers", "wlans"}
			totalSynced := 0
...
func newWorkflowStatusCmd(flags *rootFlags) *cobra.Command {
  ...
  unifi-network-pp-cli workflow status

  # Show status as JSON
  unifi-network-pp-cli workflow status --json`,
		RunE: func(cmd *cobra.Command, args []string) error {
			if dbPath == "" {
				dbPath = defaultDBPath("unifi-network-pp-cli")
			}
