...
func newDoctorCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  unifi-network-pp-cli doctor
  unifi-network-pp-cli doctor --json
  unifi-network-pp-cli doctor --fail-on warn`,
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			// Check auth. The bulk of this CLI hits site-scoped controller
			// endpoints that authenticate with the username/password cookie
			// session (UNIFI_HOST/UNIFI_USERNAME/UNIFI_PASSWORD). The Integration
			// API key (UNIFI_API_KEY) only covers the dpi/* and firewall-ordering
			// commands, so it is optional, not the primary credential.
			settings := config.LoadUnifiSettings(cfg)
			var missingCreds []string
			if settings.Host == "" {
				missingCreds = append(missingCreds, "UNIFI_HOST")
			}
			if settings.Username == "" {
				missingCreds = append(missingCreds, "UNIFI_USERNAME")
			}
			if settings.Password == "" {
				missingCreds = append(missingCreds, "UNIFI_PASSWORD")
			}
			if len(missingCreds) == 0 {
				report["auth"] = "configured (controller login)"
				report["env_vars"] = "OK UNIFI_HOST/UNIFI_USERNAME/UNIFI_PASSWORD set"
			} else {
				report["auth"] = "not configured"
				report["auth_hint"] = "set " + strings.Join(missingCreds, ", ") + " (a LOCAL controller admin, not a UI.com SSO login)"
				report["env_vars"] = "ERROR missing controller credentials: " + strings.Join(missingCreds, ", ")
			}
			if settings.APIKey != "" {
				report["integration_api_key"] = "present (dpi/* and firewall-ordering only)"
			}
			...
			if cfg != nil && cfg.BaseURL != "" {
				...
				if clientErr != nil {
					...
					// The client's BaseURL is the EFFECTIVE dial target after the
					// UNIFI_HOST override; cfg.BaseURL is the stored/default value.
					// Surfacing both makes a stale-config mismatch obvious.
					report["effective_base_url"] = c.BaseURL
					if os.Getenv("UNIFI_HOST") == "" && cfg.BaseURL == "https://192.168.1.1" {
						report["base_url_warning"] = "UNIFI_HOST not set and base_url is the default https://192.168.1.1 — set UNIFI_HOST to your controller's address"
					}
					// Step 1: Basic reachability via the configured transport.
					healthPath := "/api/s/__SITE__/self"
					...
					// Step 2: Validate the controller login with an authenticated
					// probe. /api/s/{site}/rest/user is cookie-authenticated; the
					// session transport performs the login and attaches the cookie,
					// so no auth header is set here (the Integration API key cannot
					// authenticate this controller endpoint anyway).
					if !settings.HasCookieCredentials() {
						report["credentials"] = "skipped (set UNIFI_HOST/UNIFI_USERNAME/UNIFI_PASSWORD to verify the controller login)"
					} else if reachErr != nil && !errors.As(reachErr, &reachAPIErr) {
						report["credentials"] = "skipped (controller unreachable)"
					} else {
						verifyPath := "/api/s/__SITE__/rest/user"
						_, authErr := c.Get(cmd.Context(), verifyPath, nil)
						var authAPIErr *client.APIError
						switch {
						case authErr == nil:
							report["credentials"] = "valid (controller login succeeded)"
						case errors.As(authErr, &authAPIErr):
							switch authAPIErr.StatusCode {
							case 401:
								report["credentials"] = "invalid (HTTP 401) — check UNIFI_USERNAME/UNIFI_PASSWORD (a LOCAL controller admin, not a UI.com SSO login)"
							case 403:
								report["credentials"] = "forbidden (HTTP 403) — the login worked but this admin account lacks permission for this endpoint"
							default:
								report["credentials"] = fmt.Sprintf("ok (HTTP %d from %s, login accepted)", authAPIErr.StatusCode, verifyPath)
							}
						default:
			...
			for _, key := range []string{"config_path", "base_url", "effective_base_url", "integration_api_key", "auth_source", "version"} {
...
func collectCacheReport(ctx context.Context, staleAfterSpec string) map[string]any {
	report := map[string]any{}
	dbPath := defaultDBPath("unifi-network-pp-cli")
	...
	if err != nil {
		if os.IsNotExist(err) {
			...
			report["hint"] = "Database not created yet; run 'unifi-network-pp-cli sync' to hydrate."
			return report
		}
		report["status"] = "error"
	...
	if qerr != nil {
		...
		report["status"] = "unknown"
		report["hint"] = "No sync state recorded; run 'unifi-network-pp-cli sync' to populate."
		return report
	}
	defer rows.Close()
	...
	case !haveAny && len(resources) == 0:
		report["status"] = "unknown"
		report["hint"] = "sync_state is empty; run 'unifi-network-pp-cli sync' to hydrate."
	case fresh:
		report["status"] = "fresh"
	default:
		report["status"] = "stale"
		report["oldest_age"] = oldest.Round(time.Minute).String()
		report["hint"] = "Some resources are older than stale_after; run 'unifi-network-pp-cli sync' to refresh."
	}
	return report
}
