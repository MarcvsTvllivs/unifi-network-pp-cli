...
type Config struct {
	...
	Path          string            `json:"-"`
	UnifiApiKey   string            `json:"api_key"`
}
...
func Load(configPath string) (*Config, error) {
	...
	path := configPath
	if path == "" {
		path = os.Getenv("UNIFI_NETWORK_CONFIG")
	}
	if path == "" {
		home, _ := os.UserHomeDir()
		path = filepath.Join(home, ".config", "unifi-network-pp-cli", "config.json")
	}
	cfg.Path = path
	...
	if v := os.Getenv("UNIFI_API_KEY"); v != "" {
		cfg.UnifiApiKey = v
		cfg.AuthSource = "env:UNIFI_API_KEY"
	}
	...
	if cfg.AuthSource == "" && cfg.UnifiApiKey != "" {
		cfg.AuthSource = "config"
	}
	...
	if v := os.Getenv("UNIFI_NETWORK_BASE_URL"); v != "" {
		cfg.BaseURL = v
	}
	return cfg, nil
}
...
func (c *Config) AuthHeader() string {
	...
	replacements := map[string]string{
		"token":         token,
		"api_key":       c.UnifiApiKey,
		"UNIFI_API_KEY": c.UnifiApiKey,
	}
