...
func feedbackFilePath() (string, error) {
	...
	dir := filepath.Join(home, ".local", "share", "unifi-network-pp-cli")
...
func FeedbackEndpointConfigured() bool {
	return os.Getenv("UNIFI_NETWORK_FEEDBACK_ENDPOINT") != ""
}
...
func feedbackAutoSend() bool {
	v := strings.ToLower(strings.TrimSpace(os.Getenv("UNIFI_NETWORK_FEEDBACK_AUTO_SEND")))
...
func postFeedback(url string, entry FeedbackEntry) error {
	...
	req.Header.Set("User-Agent", "unifi-network-pp-cli/feedback")
...
func newFeedbackCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Long: `Feedback is captured locally first at ~/.local/share/unifi-network-pp-cli/feedback.jsonl.
When ` + "`UNIFI_NETWORK_FEEDBACK_ENDPOINT`" + ` is set and either --send is
passed or ` + "`UNIFI_NETWORK_FEEDBACK_AUTO_SEND=true`" + `, the entry is
...
func newFeedbackListCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  unifi-network-pp-cli feedback list
  unifi-network-pp-cli feedback list --limit 5
  unifi-network-pp-cli feedback list --json`,
