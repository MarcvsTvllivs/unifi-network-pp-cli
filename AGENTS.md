# UniFi Network Printed CLI Agent Guide

This directory is a generated `unifi-network-pp-cli` printed CLI. It was produced by [CLI Printing Press](https://github.com/mvanhorn/cli-printing-press), so treat systemic fixes as upstream Printing Press fixes first. Keep local edits narrow and document why a generated-tree patch belongs here.
...
```bash
unifi-network-pp-cli doctor --json
unifi-network-pp-cli agent-context --pretty
```

Use runtime discovery instead of relying on a copied command list:

```bash
unifi-network-pp-cli which "<capability>" --json
unifi-network-pp-cli <command> --help
```
...
```bash
unifi-network-pp-cli <command> --agent
```
...
```bash
unifi-network-pp-cli <command> --help
unifi-network-pp-cli <command> --dry-run --agent
```
